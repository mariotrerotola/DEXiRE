...
from .core.dexire_abstract import (
  AbstractRuleExtractor,
  AbstractRuleSet,
  Mode,
  RuleExtractorEnum,
  TiebreakerStrategy,
...
class DEXiRE:
  ...
  def __init__(self, 
               ...
               explain_features: np.ndarray = None,
    ...
    if self.rule_extractor is None:
      ...
      if self.mode!= Mode.CLASSIFICATION and self.mode!= Mode.REGRESSION:
        raise NotImplementedError(
          f"Not implemented mode: {self.mode}. Expected Mode.CLASSIFICATION or Mode.REGRESSION."
        )
      # Check if the name of rule extractor is registered 
      if not isinstance(self.rule_extraction_method, RuleExtractorEnum):
        try:
          self.rule_extraction_method = RuleExtractorEnum(self.rule_extraction_method)
        except ValueError as exc:
          raise NotImplementedError(
            f"Rule extractor: {self.rule_extraction_method} not implemented."
          ) from exc
  ...
  def get_intermediate_model(self, layer_idx: int) -> tf.keras.Model:
    ...
    """
    model_inputs = self.model.inputs if getattr(self.model, "inputs", None) else self.model.input
    intermediate_layer_model = tf.keras.Model(inputs=model_inputs,
  ...
  def get_data_transformed(self) -> Dict[str, np.ndarray]:
  ...
  def extract_rules_at_layer(self, 
                    ...
                    random_state: int = 41,
                    cache_data: bool = True) -> List[AbstractRuleSet]:
    ...
    # retrieve prediction 
    if cache_data and "raw_prediction" in self.data_raw.keys():
    ...
    if self.mode == Mode.CLASSIFICATION:
      ...
      if pred_shape == 1:
        ...
        raise ValueError(
          f"The prediction shape cannot be processed. Expected (batch, n), obtained: {y_pred_raw.shape}"
        )
    ...
    if cache_data and layer_idx not in self.intermediate_model.keys():
    ...
    if cache_data:
      ...
      intermediate_output = self.get_intermediate_model(layer_idx=layer_idx).predict(X)
  ...
  def extract_rules(self, 
    ...
    if isinstance(self.model, tf.keras.Sequential):
      ...
      raise TypeError("The model is not a TensorFlow Sequential or Functional model and cannot be processed.")
    if layer_idx is None:
      ...
      if isinstance(layer_idx, list):
        ...
        raise TypeError("layer_idx must be a list of integers or a single integer.")
      model_layers = self.model.layers
      for idx in candidate_layers:
        if idx >= 0:
          if idx >= len(model_layers)-1:
            raise ValueError(f"Index: {idx} is out of bounds of the model")
        elif idx < 0:
          if idx < -len(model_layers):
            raise ValueError(f"Index: {idx} is out of bounds of the model")
    # extract rules from each layer 
    partial_rule_sets = []
    if n_jobs > 1 and len(candidate_layers) > 1:
      def _extract_single_layer(idx: int) -> AbstractRuleSet:
        worker = DEXiRE(
          model=self.model,
          feature_names=self.features_names,
          class_names=self.class_names,
          ...
          rule_extraction_method=self.rule_extraction_method,
          tie_breaker_strategy=self.tie_breaker_strategy
        )
        return worker.extract_rules_at_layer(
          X=X,
          y=y,
          layer_idx=idx,
          sample=sample,
      ...
      with ThreadPoolExecutor(max_workers=n_jobs) as executor:
        partial_rule_sets = list(executor.map(_extract_single_layer, candidate_layers))
    else:
  ...
  def cleanup_cache(self, keep_final_rule_set: bool = True) -> None:
    """Release cached intermediate data structures to reduce memory usage."""
    self.intermediate_rules = {}
    self.intermediate_model = {}
    self.data_raw = {}
    self.data_transformed = {}
    if not keep_final_rule_set:
      self.final_rule_set = None
